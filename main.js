...
var authFolder = storeSystem.fixFileName(`${Helper.opts._[0] || ''}VynaSession`)
...
function purgeSession() {
    let prekey = [];
    const directorio = readdirSync('./VynaSession');
    const filesFolderPreKeys = directorio.filter((file) => {
        return file.startsWith('pre-key-');
    });
    prekey = [...prekey, ...filesFolderPreKeys];
    filesFolderPreKeys.forEach((files) => {
        unlinkSync(`./VynaSession/${files}`);
    });
}
    const directories = ['./VynaSession/'];
