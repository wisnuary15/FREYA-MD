...
await conn.sendMessage(m.chat, {
  ...
  const writableStream = fs.createWriteStream(`${tmpDir}/${title}.mp4`);
  ...
  // Hapus file video
  fs.unlink(`${tmpDir}/${title}.mp4`, (err) => {
    if (err) {
      console.error(`Failed to delete video file: ${err}`);
    } else {
      console.log(`Deleted video file: ${tmpDir}/${title}.mp4`);
    }
  });
};
